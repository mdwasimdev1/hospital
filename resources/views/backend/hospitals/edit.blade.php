...
@section('content')
    ...
    <div class="max-w-xl mx-auto bg-white p-6 rounded shadow relative">
        <h2 class="text-2xl font-bold mb-4">
            {{ isset($hospital) ? 'Edit Hospital' : 'Add Hospital' }}
        </h2>
        ...
        <form method="POST"
            action="{{ isset($hospital) ? route('hospitals.update', $hospital->id) : route('hospitals.store') }}"
            ...
            <div class="mb-4">
                ...
                <input type="text" name="slug" class="w-full border px-3 py-2 rounded"
                    value="{{ old('slug', $hospital->slug ?? '') }}" required>
            </div>
            <div class="mb-4">
                ...
                <input type="text" name="title" class="w-full border px-3 py-2 rounded"
                    value="{{ old('title', $hospital->title ?? '') }}" required>
            </div>

            {{-- Contact --}}
            <div class="mb-4">
                <label class="block mb-1 font-semibold">Contact</label>
                <input type="text" name="contact" class="w-full border px-3 py-2 rounded"
                    value="{{ old('contact', $hospital->contact ?? '') }}" required>
            </div>
                ...
                <div id="locationDropdown"
                    ...
                    @foreach ($locations as $location)
                        <div class="px-3 py-2 hover:bg-blue-100 cursor-pointer"
                            onclick="selectLocation({{ $location->id }}, '{{ addslashes($location->name) }}')">
                            {{ $location->name }}
                        </div>
                    @endforeach
                </div>
            ...
            <button type="submit" class="bg-teal-400 text-white px-4 py-2 rounded hover:bg-teal-700 transition">
                {{ isset($hospital) ? 'Update' : 'Submit' }}
            </button>
        </form>
    </div>
    ...
    <script>
        ...
        let selectedLocations = @json(old('locations', isset($hospital) ? $hospital->locations->pluck('id')->toArray() : []));
        let locationMap = @json($locations->pluck('name', 'id'));
        let oldAddresses = @json(old('addresses', isset($hospital) ? $hospital->locations->pluck('pivot.address', 'id') : []));
